...
gpt3_engine = "insert_gpt3_engine_here"
...
def generate_text_for_disease(disease):
    ...
    response1 = openai.Completion.create(engine=gpt3_engine, prompt=prompt1, temperature=.3, max_tokens=64)

    prompt2 = prompt1 + response1['choices'][0]['text'] + '. Some special cases also present'
    response2 = openai.Completion.create(engine=gpt3_engine, prompt=prompt2, temperature=.3, max_tokens=128)

    prompt3 = prompt2 + response2['choices'][0]['text'] + '. The main symptomatic differences of ' + disease + ' from similar diseases is that'
    response3 = openai.Completion.create(engine=gpt3_engine, prompt=prompt3, temperature=.3, max_tokens=64)
...
def generate_text_summary(text):
    ...
    response = openai.Completion.create(engine=gpt3_engine, prompt=prompt, temperature=.25, max_tokens=50)
